...
class BlockchainTransaction(models.Model):
    """
    Tracks blockchain transactions for complaint lifecycle events.
    
    Why minimal data here?
    - The authoritative data lives on-chain
    - This is just a reference/cache for query performance
    - We store the tx_hash to verify against blockchain
    """
...
class EvidenceHash(models.Model):
    """
    Tracks locally-stored evidence with on-chain hash anchoring.
    
    Why this model?
    - Evidence files are too large for on-chain storage
    - Local storage provides simple and reliable file access
    - Blockchain provides tamper-proof hash verification
    - This model links file -> local path -> blockchain
    
    Changes from IPFS version:
    - Replaced ipfs_cid with file_path (local storage path)
    - Still maintains hash anchoring on blockchain
    - Files served via Django MEDIA_URL
    """
