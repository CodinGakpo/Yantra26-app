...
This module listens for events emitted by the smart contract and syncs them
...
- Catches any direct contract interactions

Deployment options:
1. Periodic polling (via Celery Beat) - simpler, good for low-volume
2. WebSocket subscription - real-time, more complex
3. AWS EventBridge + Lambda - serverless, scalable
"""
...
def sync_events_from_blockchain(
    ...
    This function queries the blockchain for events and updates the local database.
    
    Args:
        from_block: Starting block number (None = last synced)
...
# ============ Real-time WebSocket Listener (Advanced) ============

class BlockchainEventListener:
    """
    Real-time event listener using WebSocket.
    
    Usage:
        listener = BlockchainEventListener()
        listener.start()
    
    Note: Requires WebSocket provider (wss://) in settings
    """
...
def run_event_listener_daemon():
    ...
    Usage:
        python manage.py listen_blockchain_events
    """
    listener = BlockchainEventListener()
    listener.start()
